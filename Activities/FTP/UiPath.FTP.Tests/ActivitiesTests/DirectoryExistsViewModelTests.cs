...
{
    /// <summary>
    /// Coverage for <see cref="DirectoryExistsViewModel.InitializeModel"/>: property configuration
    /// only, no rules, no dependencies, no data sources.
    /// </summary>
    public class DirectoryExistsViewModelTests
    {
        private static DirectoryExistsViewModel NewViewModel() =>
            new DirectoryExistsViewModel(Mock.Of<IDesignServices>());
