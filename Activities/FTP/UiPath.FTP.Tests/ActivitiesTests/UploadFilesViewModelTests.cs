...
{
    /// <summary>
    /// Coverage for <see cref="UploadFilesViewModel.InitializeModel"/>: property configuration only,
    /// no rules, no dependencies, no data sources.
    /// </summary>
    public class UploadFilesViewModelTests
    {
        private static UploadFilesViewModel NewViewModel() =>
            new UploadFilesViewModel(Mock.Of<IDesignServices>());

        [Fact]
        public async Task InitializeAsync_ConfiguresLocalPathBeforeRemotePath()
        {
            var vm = NewViewModel();
            ...
            Assert.Equal(1, vm.LocalPath.OrderIndex);
            ...
            Assert.Equal(2, vm.RemotePath.OrderIndex);
        }
